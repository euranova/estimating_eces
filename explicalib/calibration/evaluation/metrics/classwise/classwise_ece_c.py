...
import numpy as np
from ..specific.specific_ece_c import specific_ece_c


def classwise_ece_c(model=None, X=None, scores=None, Y=None, n_bins=10, backend=None):
    """
    Calculates the SCE (with convex allocation) of the model based on data (X,Y).
    Args:
    ...
    if backend == "accuracies_confidences":
        ...
        for i in range(scores.shape[1]):
            result += specific_ece_c(model=model, specific_scores=scores[:, i], Y=Y, n_bins=n_bins,
