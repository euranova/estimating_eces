...
import os
from ....calibration.evaluation.metrics.confidence.confidence_ece import confidence_ece
from ....calibration.evaluation.metrics.confidence.confidence_ece_a import confidence_ece_a
from ....calibration.evaluation.metrics.confidence.confidence_ece_c import confidence_ece_c
from ....calibration.evaluation.metrics.confidence.confidence_ece_ac import confidence_ece_ac
from ....calibration.evaluation.metrics.confidence.direct_confidence_ece import direct_confidence_ece
...
evaluated_metric_payloads = [
    ...
    (direct_confidence_ece, {"bandwidth": "silverman"}),
    (direct_confidence_ece, {"bandwidth": 0.1}),
    (direct_confidence_ece, {"bandwidth": 0.033}),

    (confidence_ece, {"n_bins": 10}),
    (confidence_ece, {"n_bins": 30}),
    (confidence_ece, {"n_bins": "sqrt"}),

    (confidence_ece_a, {"n_bins": 10}),
    (confidence_ece_a, {"n_bins": 30}),
    (confidence_ece_a, {"n_bins": "sqrt"}),

    (confidence_ece_c, {"n_bins": 10}),
    (confidence_ece_c, {"n_bins": 30}),
    (confidence_ece_c, {"n_bins": "sqrt"}),

    (confidence_ece_ac, {"n_bins": 10}),
    (confidence_ece_ac, {"n_bins": 30}),
    (confidence_ece_ac, {"n_bins": "sqrt"}),
...
def figure_confidence_distribution(confidences):
    # Creating figure
    fig = plt.figure(figsize=(11, 5))
    gs = fig.add_gridspec(1, 1)

    ax = fig.add_subplot(gs[0, 0])
    plot_scores_distribution(scores=confidences, target_axis=ax)
            ...
            for seed_distribution in seeds_distribution:
                ...
                for seed_train_test_split in range(N_TRAIN_TEST_SPLITS):
                    ...
                    for model_family in [SVCc, GaussianNB, LogisticRegression, RandomForestClassifier]:
                        ...
                        predictions_holdout = np.argmax(scores_holdout, axis=1)
                        confidences_holdout = confidences_from_scores(scores_matrix=scores_holdout,
                                                                      predictions=predictions_holdout,
                                                                      model=model)

                        # Saving scores distribution
                        scores_dist = figure_confidence_distribution(confidences_holdout)
                        fig_name = \
                        ...
                        savefig(setup_name=setup_name,
                                fig_dir=os.path.join("shared", "confidences_distributions", str(n_features_i),
                                                     str(n_classes_i)), fig_name=fig_name, fig=scores_dist)

                        # Calculating a very precise estimate of the ECE
                        gt = confidence_ece(model=model,
                                            confidence_scores=confidences_holdout,
                                            predictions=predictions_holdout,
                                            Y=Y_holdout,
                                            n_bins=N_BINS_GT)
                        ...
                        for metric, kwargs in evaluated_metric_payloads:
                            ...
                            for n_samples_eval in tqdm(n_samples_calibration_evaluations, postfix=identifier):
                                ...
                                # distances_to_gt_n_samples = []

                                for _ in range(N_SAMPLINGS):
                                    # Getting evaluation set
                                    scores_eval, predictions_eval, Y_eval = resample(
                                        confidences_holdout, predictions_holdout, Y_holdout,
                                        n_samples=n_samples_eval,
                                        random_state=reproducible_train_test_split)

                                    # Calculating evaluated metric value
                                    metric_value = metric(confidence_scores=scores_eval,
                                                          predictions=predictions_eval,
                                                          Y=Y_eval, model=model, **kwargs)
                                    metric_values_n_samples.append(metric_value)
                                    # distances_to_gt_n_samples.append(np.abs(metric_value - gt))

                                metric_values.append(metric_values_n_samples)
                                # distances_to_gt.append(distances_to_gt_n_samples)
